...
function section(title,items,suffix="",filter="",layout="row"){
  ...
  return `<section class="row"><div class="row-head"><div class="row-heading"><span class="row-eyebrow">NEXORA</span><h2>${title}</h2></div><button class="row-link" data-row-filter="${filter}">${suffix||`${count} titre${count>1?"s":""}`} <span>→</span></button></div><div class="row-cards-shell"><button class="row-scroll-control row-scroll-left" type="button" data-row-scroll="-1" aria-label="Défiler ${escapeAttr(title)} vers la gauche">‹</button><div class="${listClass}" >${visible.map(card).join("")}</div><span class="row-edge-fade row-edge-fade-right" aria-hidden="true"></span><button class="row-scroll-control row-scroll-right" type="button" data-row-scroll="1" aria-label="Défiler ${escapeAttr(title)} vers la droite">›</button><span class="row-edge-fade row-edge-fade-left" aria-hidden="true"></span></div></section>`;
}
