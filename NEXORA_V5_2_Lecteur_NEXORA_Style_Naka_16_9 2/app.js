...
function getVidzyUrl(item, season=1, episode=1){
  const tmdbId=item?.tmdb_id;
  if(!tmdbId)return "";
  const type=String(item.type||item.media_type||"").toLowerCase();
  const isSeries=["serie","series","tv","anime","show"].includes(type);
  return isSeries
    ? `https://vidzy.org/serie/${encodeURIComponent(tmdbId)}/${season}/${episode}`
    : `https://vidzy.org/movie/${encodeURIComponent(tmdbId)}`;
}
...
function populateEpisodeControls(item, initialSeason=1, initialEpisode=1){
  ...
  const update=()=>{
    ...
    const url=getVidzyUrl(item,season,episode),frame=$("playerFrame");
    updatePlayerEpisodeInfo(item,season,episode);
    if(url){frame.src=url;frame.classList.remove("hidden");$("playerEmpty").classList.add("hidden");$("playerVideo").classList.add("hidden");renderPlayerSources(item,season,episode);}
  };
...
function getPlayerSources(item, season=1, episode=1){
  const sources=[];
  const vidzy=getVidzyUrl(item,season,episode);
  if(vidzy) sources.push({id:"vidzy",name:"Vidzy",type:"iframe",url:vidzy});
  if(item?.video_url) sources.push({id:"direct",name:"Source directe",type:"video",url:item.video_url});
  if(Array.isArray(item?.sources)) item.sources.forEach((source,index)=>{
    if(!source?.url) return;
    sources.push({id:String(source.id||`source-${index+1}`),name:source.name||`Source ${index+1}`,type:source.type||"iframe",url:source.url});
  });
  return sources;
}
function renderPlayerSources(item,season=1,episode=1){
  ...
  bar.innerHTML=sources.length?sources.map((source,index)=>`<button type="button" class="player-source ${index===0?'active':''}" data-player-source="${escapeAttr(source.id)}">${escapeHtml(source.name)}</button>`).join(""):'<span class="player-source-empty">Aucune source disponible</span>';
  bar.querySelectorAll("[data-player-source]").forEach(button=>button.addEventListener("click",()=>{
    ...
    const video=$("playerVideo"),frame=$("playerFrame"),empty=$("playerEmpty");
    if(source.type==="video"){
      frame.removeAttribute("src");frame.classList.add("hidden");video.classList.remove("hidden");empty.classList.add("hidden");video.src=source.url;video.play().catch(()=>{});
    }else{
      video.pause();video.removeAttribute("src");video.load();video.classList.add("hidden");empty.classList.add("hidden");frame.src=source.url;frame.classList.remove("hidden");
    }
  }));
}
function openPlayer(id, initialSeason=1, initialEpisode=1){
  ...
  $("playerPoster").style=bgStyle(item);
  $("playerModal").classList.remove("hidden");
  $("playerModal").setAttribute("aria-hidden","false");
  document.body.classList.add("player-open");
  /* Le lecteur reste intégré dans la page. Le plein écran est déclenché uniquement par le bouton dédié. */
  populateEpisodeControls(item, initialSeason, initialEpisode);
  renderPlayerSources(item,initialSeason,initialEpisode);
  hydratePlayerSeries(item,initialSeason,initialEpisode);
  const sources=getPlayerSources(item,initialSeason,initialEpisode),source=sources[0];
  if(source){
    empty.classList.add("hidden");
    if(source.type==="video"){
      frame.removeAttribute("src");frame.classList.add("hidden");video.classList.remove("hidden");video.src=source.url;video.play().catch(()=>{});
    }else{
      video.pause();video.removeAttribute("src");video.load();video.classList.add("hidden");frame.src=source.url;frame.classList.remove("hidden");
    }
  }else{
    frame.removeAttribute("src");frame.classList.add("hidden");video.pause();video.removeAttribute("src");video.load();video.classList.add("hidden");empty.classList.remove("hidden");
  }
}
function closePlayer(){const video=$("playerVideo");video.pause();video.removeAttribute("src");video.load();document.body.classList.remove("player-open");if(document.fullscreenElement && document.exitFullscreen){document.exitFullscreen().catch(()=>{});}$("playerModal").classList.add("hidden");$("playerModal").setAttribute("aria-hidden","true");}
...
async function loadContents(){
  ...
  contents=(dbContents.length?dbContents:[...demoCatalog,...extraCatalog]).map(normalizeContent);
  ...
  syncCatalogIfNeeded().then(async result=>{
    ...
    try{
      ...
      if(refreshed.length){
        contents=refreshed.map(normalizeContent);
